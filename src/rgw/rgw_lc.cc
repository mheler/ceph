...
// Batched LC action counter accumulator
struct LCBatchCounters {
  ceph::mutex lock = ceph::make_mutex("LCBatchCounters");
  std::string bucket_name;
  uint64_t expire_current = 0;
  uint64_t expire_noncurrent = 0;
  uint64_t expire_dm = 0;
  uint64_t transition_current = 0;
  uint64_t transition_noncurrent = 0;
  uint64_t abort_mpu = 0;
  uint64_t obj_pending_decrements = 0;
  uint64_t ops_since_flush = 0;
  uint64_t flush_threshold;

  LCBatchCounters(const std::string& bucket, uint64_t threshold)
    : bucket_name(bucket), flush_threshold(threshold) {}

  void increment(int counter_idx) {
    std::lock_guard l(lock);
    switch (counter_idx) {
      case l_rgw_lc_per_bucket_expire_current:
        expire_current++;
        break;
      case l_rgw_lc_per_bucket_expire_noncurrent:
        expire_noncurrent++;
        break;
      case l_rgw_lc_per_bucket_expire_dm:
        expire_dm++;
        break;
      case l_rgw_lc_per_bucket_transition_current:
        transition_current++;
        break;
      case l_rgw_lc_per_bucket_transition_noncurrent:
        transition_noncurrent++;
        break;
      case l_rgw_lc_per_bucket_abort_mpu:
        abort_mpu++;
        break;
    }
    ops_since_flush++;
    if (flush_threshold > 0 && ops_since_flush >= flush_threshold) {
      flush_locked();
    }
  }

  void decrement(int counter_idx) {
    std::lock_guard l(lock);
    if (counter_idx == l_rgw_lc_per_bucket_obj_pending) {
      obj_pending_decrements++;
    }
  }
...
private:
  void flush_locked() {
    if (expire_current > 0) {
      rgw::lc_counters::inc(bucket_name, l_rgw_lc_per_bucket_expire_current, expire_current);
      expire_current = 0;
    }
    if (expire_noncurrent > 0) {
      rgw::lc_counters::inc(bucket_name, l_rgw_lc_per_bucket_expire_noncurrent, expire_noncurrent);
      expire_noncurrent = 0;
    }
    if (expire_dm > 0) {
      rgw::lc_counters::inc(bucket_name, l_rgw_lc_per_bucket_expire_dm, expire_dm);
      expire_dm = 0;
    }
    if (transition_current > 0) {
      rgw::lc_counters::inc(bucket_name, l_rgw_lc_per_bucket_transition_current, transition_current);
      transition_current = 0;
    }
    if (transition_noncurrent > 0) {
      rgw::lc_counters::inc(bucket_name, l_rgw_lc_per_bucket_transition_noncurrent, transition_noncurrent);
      transition_noncurrent = 0;
    }
    if (abort_mpu > 0) {
      rgw::lc_counters::inc(bucket_name, l_rgw_lc_per_bucket_abort_mpu, abort_mpu);
      abort_mpu = 0;
    }
    if (obj_pending_decrements > 0) {
      rgw::lc_counters::dec(bucket_name, l_rgw_lc_per_bucket_obj_pending, obj_pending_decrements);
      obj_pending_decrements = 0;
    }
    ops_since_flush = 0;
  }
};
...
struct lc_op_ctx {
  ...
  lc_op_ctx(op_env& env, rgw_bucket_dir_entry& o,
    ...
    : cct(env.driver->ctx()), env(env), o(o), next_key_name(next_key_name),
      ...
      octx(env.driver), dpp(dpp), wq(wq), batch_counters(batch_counters)
    {
...
public:
  ...
  int process(rgw_bucket_dir_entry& o, const DoutPrefixProvider *dpp,
	      WorkQ* wq, LCBatchCounters* batch_counters);
}; /* LCOpRule */
				       ...
				       LCBatchCounters* batch_counters)
{
  ...
  auto pf = [&, batch_counters](RGWLC::LCWorker *wk, WorkQ *wq, WorkItem &wi) {
    int ret{0};
	...
	return ret;
      ...
      if (ret == 0) {
        ...
        batch_counters->increment(l_rgw_lc_per_bucket_abort_mpu);
      } else {
...
public:
  ...
  int process(lc_op_ctx& oc) override {
	...
	return r;
      ...
      oc.batch_counters->increment(l_rgw_lc_per_bucket_expire_current);
...
public:
  ...
  int process(lc_op_ctx& oc) override {
    ...
    oc.batch_counters->increment(l_rgw_lc_per_bucket_expire_noncurrent);
    ldpp_dout(oc.dpp, 2) << "DELETED:" << oc.bucket << ":" << o.key
...
public:
  ...
  int process(lc_op_ctx& oc) override {
    ...
    oc.batch_counters->increment(l_rgw_lc_per_bucket_expire_dm);
    ldpp_dout(oc.dpp, 2) << "DELETED:" << oc.bucket << ":" << o.key
  ...
  LCOpAction_CurrentTransition(const transition_action& _transition)
    ...
    int process(lc_op_ctx& oc) override {
      ...
      if (r == 0) {
        ...
        oc.batch_counters->increment(l_rgw_lc_per_bucket_transition_current);
      }
      return r;
    }
};
  ...
  LCOpAction_NonCurrentTransition(op_env& env,
    ...
    int process(lc_op_ctx& oc) override {
      ...
      if (r == 0) {
        ...
        oc.batch_counters->increment(l_rgw_lc_per_bucket_transition_noncurrent);
      }
      return r;
    }
};
...
int LCOpRule::process(rgw_bucket_dir_entry& o,
		      ...
		      WorkQ* wq, LCBatchCounters* batch_counters)
{
  lc_op_ctx ctx(env, o, next_key_name, num_noncurrent, effective_mtime, dpp, wq, batch_counters);
			     ...
			     time_t stop_at, bool once)
{
  ...
  // Reset LC counters at START of bucket processing
  rgw::lc_counters::set(bucket->get_name(), l_rgw_lc_per_bucket_expire_current, 0);
  rgw::lc_counters::set(bucket->get_name(), l_rgw_lc_per_bucket_expire_noncurrent, 0);
  rgw::lc_counters::set(bucket->get_name(), l_rgw_lc_per_bucket_expire_dm, 0);
  rgw::lc_counters::set(bucket->get_name(), l_rgw_lc_per_bucket_transition_current, 0);
  rgw::lc_counters::set(bucket->get_name(), l_rgw_lc_per_bucket_transition_noncurrent, 0);
  rgw::lc_counters::set(bucket->get_name(), l_rgw_lc_per_bucket_abort_mpu, 0);
  rgw::lc_counters::set(bucket->get_name(), l_rgw_lc_per_bucket_obj_total, 0);
  rgw::lc_counters::set(bucket->get_name(), l_rgw_lc_per_bucket_obj_pending, 0);

  uint64_t total_objects_processed = 0;

  // Create batched counter accumulator
  uint64_t batch_threshold = cct->_conf->rgw_lc_counters_batch_size;
  LCBatchCounters batch_counters(bucket->get_name(), batch_threshold);

  auto pf = [&bucket_name, &batch_counters](RGWLC::LCWorker* wk, WorkQ* wq, WorkItem& wi) {
    auto wt =
    ...
    ldpp_dout(wk->get_lc(), 20)
      ...
      << dendl;
    int ret = op_rule.process(o, wk->dpp, wq, &batch_counters);
    if (ret < 0) {
      ldpp_dout(wk->get_lc(), 20)
    ...
    // Decrement pending counter after processing each object
    batch_counters.decrement(l_rgw_lc_per_bucket_obj_pending);
  };
  worker->workpool->setf(pf);
  ...
  for(auto prefix_iter = prefix_map.begin(); prefix_iter != prefix_map.end();
    ...
    for (auto offset = 0; ol.get_obj(this, &o /* , fetch_barrier */); ++offset, ol.next()) {
      ...
      // Update obj_pending after each object is enqueued (before processing starts)
      rgw::lc_counters::inc(bucket->get_name(), l_rgw_lc_per_bucket_obj_pending, 1);
  ...
  // Set objects_total to show total objects enumerated
  rgw::lc_counters::set(bucket->get_name(), l_rgw_lc_per_bucket_obj_total, total_objects_processed);

  // Flush any remaining batched counters after workpool drains
  batch_counters.flush();

  ret = handle_multipart_expiration(bucket.get(), prefix_map, worker, stop_at, once, &batch_counters);

  // Flush again after multipart expiration
  batch_counters.flush();

  // Update LC timestamp at END of bucket processing (Unix epoch seconds)
  rgw::lc_counters::set(bucket->get_name(), l_rgw_lc_per_bucket_last_proc, ceph_clock_now().sec());
