...
inline uint64_t gcm_encrypted_to_plaintext_size(uint64_t encrypted_size,
  ...
  if (remainder > 0 && remainder <= AES_256_GCM_TAG_SIZE) {
    // Malformed: partial chunk has no ciphertext, only tag bytes
    if (dpp) {
      ldpp_dout(dpp, 1) << "WARNING: gcm_encrypted_to_plaintext_size: "
          << "partial chunk size " << remainder
          << " is <= tag size " << AES_256_GCM_TAG_SIZE
          << " - data may be corrupted" << dendl;
    }
    return full_chunks * AES_256_GCM_CHUNK_SIZE;
  }
...
/**
 * Convert plaintext size to encrypted size for GCM mode.
 * Each 4096-byte plaintext chunk becomes 4112 bytes (with 16-byte auth tag).
 */
inline uint64_t gcm_plaintext_to_encrypted_size(uint64_t plaintext_size) {
  if (plaintext_size == 0) return 0;
  uint64_t num_chunks = (plaintext_size + AES_256_GCM_CHUNK_SIZE - 1) / AES_256_GCM_CHUNK_SIZE;
  return plaintext_size + (num_chunks * AES_256_GCM_TAG_SIZE);
}

/**
 * Convert plaintext offset to encrypted offset for GCM mode.
 * Accounts for 16-byte auth tag per chunk.
 */
inline uint64_t gcm_plaintext_to_encrypted_offset(uint64_t plaintext_ofs) {
  uint64_t chunk_idx = plaintext_ofs / AES_256_GCM_CHUNK_SIZE;
  uint64_t offset_in_chunk = plaintext_ofs % AES_256_GCM_CHUNK_SIZE;
  return chunk_idx * AES_256_GCM_ENCRYPTED_CHUNK_SIZE + offset_in_chunk;
}
...
class RGWGetObj_BlockDecrypt : public RGWGetObj_Filter {
  ...
  uint32_t starting_part_num = 0; /**< first part number from manifest (for GET ?partNumber=N) */
  ...
  off_t logical_to_encrypted_offset(off_t logical_ofs) const {
    if (block_size == encrypted_block_size) {
      return logical_ofs; // CBC - no conversion needed
    }
    return gcm_plaintext_to_encrypted_offset(logical_ofs);
  }
  ...
  size_t encrypted_to_plaintext_size(size_t encrypted_size) const {
    ...
    return gcm_encrypted_to_plaintext_size(encrypted_size, dpp);
  }
...
public:
  ...
  RGWGetObj_BlockDecrypt(const DoutPrefixProvider *dpp,
    ...
    : RGWGetObj_BlockDecrypt(dpp, cct, next, std::move(crypt),
                             std::move(parts_len), 0, 0, false, y) {}
  ...
  static uint32_t read_manifest_starting_part(const DoutPrefixProvider *dpp,
                                              const bufferlist& manifest_bl);
 ...
 * Returns false if attr missing, not GCM mode, or parse failure.
 */
bool rgw_get_gcm_original_size(const DoutPrefixProvider* dpp,
 ...
 */
bool rgw_get_gcm_decrypted_size(const DoutPrefixProvider* dpp,
                                const std::map<std::string, ceph::bufferlist>& attrs,
