...
static BenchResult run_bench(const string& label, size_t data_size,
  ...
  auto thread_body = [&](int tid) {
    auto crypt = factory();
    if (!crypt) { failed.store(true); return; }
    for (int i = 0; i < warmup_iters; ++i) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if ((arg == "--size" || arg == "-s") && i + 1 < argc) {
      ...
      cout << "Usage: " << argv[0] << " [options]\n"
           ...
           << "  --iterations, -n <N>   Fixed iteration count (default: auto-scale)\n"
  ...
  for (size_t sz : sizes) {
    ...
    if (have_gcm) {
...

      {
        ...
        bufferlist ct;
        pre->encrypt(input, 0, sz, ct, 0, null_yield);
        string nonce = AES_256_GCM_get_nonce(pre.get());
        size_t enc_sz = ct.length();
        auto dec_factory = [&, nonce]() {
          return AES_256_GCM_create(&no_dpp, g_ceph_context, key, AES_256_KEYSIZE,
