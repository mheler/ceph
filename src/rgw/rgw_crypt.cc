                                 ...
                                 std::unique_ptr<BlockCrypt>* block_crypt)
{
  *block_crypt = nullptr;
  ...
  std::string mode = get_str_attribute(dest_attrs, RGW_ATTR_CRYPT_MODE);
  if (mode.empty()) {
    return 0;
  }
  ...
  if (!dest_mode.empty()) {
    /*
     * Write the mode before building anything. Mode dispatch, key
     * fetch and the aead key derivation domain all read it from here,
     * so they stay in step with what a later read will see.
     */
