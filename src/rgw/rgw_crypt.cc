...
public:
  ...
  void prepare_iv(unsigned char (&iv)[AES_256_IVSIZE], off_t offset) {
    ...
    // Validate chunk_index fits in 40 bits (supports up to 4 PB per part at 4KB chunks)
    ceph_assert((chunk_index >> 40) == 0 && "chunk_index exceeds 40 bits, IV collision risk");
...
int RGWGetObj_BlockDecrypt::fixup_range(off_t& bl_ofs, off_t& bl_end) {
  ...
  if (parts_len.size() > 0) {
    ...
    // For GCM: ensure we read complete encrypted blocks (including auth tags)
    if (size_expanding) {
      enc_rounded_end = (enc_rounded_end / encrypted_block_size) * encrypted_block_size + (encrypted_block_size - 1);
    }
  ...
  else
  {
    ...
    // Clamp to actual encrypted object size (critical for small objects)
    ...
    if (! req_sse_ca.empty()) {
      ...
      if (use_gcm) {
        ...
        if (s->content_length > 0) {
          set_attr(attrs, RGW_ATTR_CRYPT_PLAINTEXT_SIZE, std::to_string(s->content_length));
        }
