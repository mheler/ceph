...
#define DEFAULT_MULTIPART_SYNC_PART_SIZE (32 * 1024 * 1024)
  ...
  bool delete_with_head_object{false};
  ...
  void encode(bufferlist& bl) const {
    ...
    encode(delete_with_head_object, bl);
  ...
  void decode(bufferlist::const_iterator& bl) {
    ...
    if (struct_v >= 4) {
      decode(delete_with_head_object, bl);
    }
...
#define DEFAULT_READ_THROUGH_RESTORE_DAYS 1
  ...
  bool retain_versioned_head = false;
  ...
  void encode(bufferlist& bl) const {
    ...
    encode(retain_versioned_head, bl);
    ENCODE_FINISH(bl);
  }
  ...
  void decode(bufferlist::const_iterator& bl) {
    ...
    if (struct_v >= 5) {
      decode(retain_versioned_head, bl);
    }
    DECODE_FINISH(bl);
  }
