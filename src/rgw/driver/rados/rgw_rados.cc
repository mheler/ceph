                                 ...
                                 bool treat_lc_defer_as_deleted)
{
  ...
  *psm = sm;
  ...
  // TEST PATCH: Skip guard reads to isolate overhead source
  // If overhead disappears with this patch, it's the guard READS causing slowdown
  // If overhead remains, it's the guard WRITES (or something else)
  constexpr bool skip_guard_check_for_testing = true;
  if (!skip_guard_check_for_testing &&
      treat_lc_defer_as_deleted && gc && cct->_conf->rgw_lc_defer_delete) {
    /* Use same tag selection as DeferredDelete::prepare(): prefer tail_tag
     * when present (multipart objects), otherwise obj_tag. This ensures we
     * query the same GC shard where the guard was stored.
     */
    std::string current_tag;
    if (s->tail_tag.length() > 0) {
      current_tag = s->tail_tag.to_str();
    } else if (s->obj_tag.length() > 0) {
      current_tag = s->obj_tag.to_str();
    }
    if (!current_tag.empty()) {
      /* Guard key must match what's used in send_chain(): raw_head.oid from obj_to_raw()
       * which includes bucket.marker prefix via get_obj_bucket_and_oid_loc()
       */
      std::string guard_oid;
      std::string guard_loc;
      get_obj_bucket_and_oid_loc(obj, guard_oid, guard_loc);
      bool guarded = false;
      int guard_ret = gc->guard_exists(guard_oid, current_tag, &guarded, y);
