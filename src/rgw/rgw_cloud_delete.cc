...
/** Delete cloud object.
 *  Always tries the versioned name first. Falls back to head name only after
 *  validating identity via HEAD (or for legacy objects without version-id
 *  metadata). This avoids relying on stale enqueue-time "current" state. */
int handle_entry(const DoutPrefixProvider *dpp, rgw::sal::Driver* driver,
  ...
  if (ret == -ENOENT && head_name != versioned_name) {
    // The head name may now belong to another version. Validate
    // x-amz-meta-rgwx-source-version-id before deleting.
    std::string resource = entry.target_bucket_name + "/" + head_name;
    std::map<std::string, std::string> resp_headers;
    int hret = head_resource(dpp, conn, resource, resp_headers, y);
    if (hret == 0) {
      // RGW HTTP client normalizes headers to uppercase with underscores
      auto it = resp_headers.find("X_AMZ_META_RGWX_SOURCE_VERSION_ID");
      if (it != resp_headers.end()) {
        if (it->second == entry.src_version_id) {
          ret = try_delete(head_name);
        }
        // else: version mismatch — belongs to a different version, skip
      } else {
        // No version-id header: transitioned before versioning metadata
        // was written. Keep legacy behavior and delete head fallback.
        ret = try_delete(head_name);
      }
    } else if (hret != -ENOENT) {
      // Transient HEAD failure — propagate so the entry is retried
      // rather than dropped as terminal success (-ENOENT)
      ret = hret;
    }
  }
  return ret;
  ...
  for (int i = 0; i < max_objs; i++) {
    ...
    do {
      ...
      if (elapsed > std::chrono::seconds(lock_secs / 2)) {
        int rret = serializer->renew_lock(this, lock_dur, y);
        if (rret < 0) {
