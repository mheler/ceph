...
// Mock that simulates GCM size expansion (encrypted_block_size > block_size)
class BlockCryptNoneGCM: public BlockCryptNone {
public:
  BlockCryptNoneGCM() : BlockCryptNone(AES_256_GCM_CHUNK_SIZE) {}
  size_t get_encrypted_block_size() override
  {
    return AES_256_GCM_ENCRYPTED_CHUNK_SIZE;  // 4096 + 16 = 4112
  }
};
...
TEST(TestRGWCrypto, verify_AES_256_GCM_identity)
{
  ...
  for (unsigned int step : {1, 2, 3, 5, 7, 11, 13, 17})
  {
    ...
    for (size_t r = 97; r < 123 ; r++)
    {
      ...
      size_t expected_encrypted_size = gcm_plaintext_to_encrypted_size(end - begin);
...
TEST(TestRGWCrypto, verify_AES_256_GCM_chunk_sizes)
{
  ...
  for (size_t size : {0, 1, 16, 4095, 4096, 4097, 8192, 10000})
  {
    ...
    size_t expected_size = gcm_plaintext_to_encrypted_size(size);
...
{
  // Test is_gcm_mode() - requires "-GCM" suffix
  ASSERT_TRUE(is_gcm_mode("SSE-C-AES256-GCM"));
  ASSERT_TRUE(is_gcm_mode("SSE-KMS-GCM"));
  ASSERT_TRUE(is_gcm_mode("RGW-AUTO-GCM"));
  ASSERT_TRUE(is_gcm_mode("AES256-GCM"));
  ASSERT_FALSE(is_gcm_mode("SSE-C-AES256"));
  ASSERT_FALSE(is_gcm_mode("SSE-KMS"));
  ASSERT_FALSE(is_gcm_mode(""));
  ASSERT_FALSE(is_gcm_mode("GCM"));
  ...
  // Test gcm_plaintext_to_encrypted_size()
  ASSERT_EQ(gcm_plaintext_to_encrypted_size(0), 0UL);       // Zero bytes
  ASSERT_EQ(gcm_plaintext_to_encrypted_size(1), 17UL);      // 1 + 16 tag
  ASSERT_EQ(gcm_plaintext_to_encrypted_size(100), 116UL);   // 100 + 16 tag
  ASSERT_EQ(gcm_plaintext_to_encrypted_size(4095), 4111UL); // 4095 + 16 tag
  ASSERT_EQ(gcm_plaintext_to_encrypted_size(4096), 4112UL); // Full chunk
  ASSERT_EQ(gcm_plaintext_to_encrypted_size(4097), 4129UL); // 4097 + 32 tags (2 chunks)
  ASSERT_EQ(gcm_plaintext_to_encrypted_size(8192), 8224UL); // Two full chunks

  // Test gcm_encrypted_to_plaintext_size()
  ASSERT_EQ(gcm_encrypted_to_plaintext_size(0), 0UL);       // Zero bytes
  ASSERT_EQ(gcm_encrypted_to_plaintext_size(16), 0UL);      // Tag only -> 0 plaintext
  ASSERT_EQ(gcm_encrypted_to_plaintext_size(17), 1UL);      // 1 byte + tag
  ASSERT_EQ(gcm_encrypted_to_plaintext_size(100), 84UL);    // 100 - 16 tag
  ASSERT_EQ(gcm_encrypted_to_plaintext_size(4112), 4096UL); // Full chunk
  ASSERT_EQ(gcm_encrypted_to_plaintext_size(4212), 4180UL); // 4096 + 84 (one chunk + partial)
  ASSERT_EQ(gcm_encrypted_to_plaintext_size(8224), 8192UL); // Two full chunks
  ...
  for (uint64_t plain : {0UL, 1UL, 16UL, 100UL, 4095UL, 4096UL, 4097UL,
    ...
    uint64_t enc = gcm_plaintext_to_encrypted_size(plain);
    uint64_t recovered = gcm_encrypted_to_plaintext_size(enc);
    ASSERT_EQ(plain, recovered)
      << "Roundtrip failed for plaintext=" << plain
      << " encrypted=" << enc << " recovered=" << recovered;
  }

  // Test gcm_plaintext_to_encrypted_offset()
  ASSERT_EQ(gcm_plaintext_to_encrypted_offset(0), 0UL);       // Start of file
  ASSERT_EQ(gcm_plaintext_to_encrypted_offset(100), 100UL);   // Within first chunk
  ASSERT_EQ(gcm_plaintext_to_encrypted_offset(4095), 4095UL); // End of first chunk
  ASSERT_EQ(gcm_plaintext_to_encrypted_offset(4096), 4112UL); // Start of second chunk
  ASSERT_EQ(gcm_plaintext_to_encrypted_offset(4196), 4212UL); // 100 bytes into second chunk
  ASSERT_EQ(gcm_plaintext_to_encrypted_offset(8192), 8224UL); // Start of third chunk
  ...
  // GCM expands data (adds 16-byte auth tag per chunk), CBC does not
  {
    ...
    auto gcm = std::make_unique<BlockCryptNoneGCM>();
...
TEST(TestRGWCrypto, verify_PartLocation_multipart_gcm)
{
  const NoDoutPrefix no_dpp(g_ceph_context, dout_subsys);
  ut_get_sink get_sink;

  // For GCM, parts_len contains ENCRYPTED sizes (with auth tag overhead)
  const size_t plaintext_per_part = 1024 * AES_256_GCM_CHUNK_SIZE;  // 4MB
  const size_t encrypted_per_part = gcm_plaintext_to_encrypted_size(plaintext_per_part);
  std::vector<size_t> parts = {encrypted_per_part, encrypted_per_part, encrypted_per_part};

  auto crypt = std::make_unique<BlockCryptNoneGCM>();
  TestableBlockDecrypt decrypt(&no_dpp, g_ceph_context, &get_sink,
                               std::move(crypt), parts);
