                                           ...
                                           jspan_context& trace, bool log_op)
{
  ...
  // For GCM: hide encryption overhead, preserve compression savings
  uint64_t index_size, index_accounted_size;
  {
    index_size = size;
    index_accounted_size = accounted_size;
    uint64_t original_size = 0;
    if (rgw_get_gcm_original_size(rctx.dpp, attrs, &original_size)) {
      // GCM+compression: decrypted size for disk, original for quota
      uint64_t compressed_size = 0;
      rgw_get_gcm_decrypted_size(rctx.dpp, attrs, size, &compressed_size);
      index_size = compressed_size;
      index_accounted_size = original_size;
    } else {
      // Check for GCM without compression (hide overhead)
      uint64_t decrypted_size = 0;
      if (rgw_get_gcm_decrypted_size(rctx.dpp, attrs, size, &decrypted_size)) {
        index_size = decrypted_size;
        index_accounted_size = decrypted_size;
      }
      // else: not GCM, keep original values
    }
  }
               ...
               jspan_context& trace)
{
  ...
  // note: CRYPT_ORIGINAL_SIZE and CRYPT_PARTS are preserved for size calculations
                                 ...
                                 optional_yield y, bool assume_noent)
{
  ...
  *psm = sm;
  ...
  /**
   * For GCM encryption: adjust accounted_size to original size.
   * Helpers return false for non-GCM modes (including CBC), so this is a no-op
   * outside of GCM.
  ...
  if (!compressed && s->accounted_size == s->size) {
    uint64_t original_size = 0;
    if (rgw_get_gcm_original_size(dpp, s->attrset, &original_size)) {
      s->accounted_size = original_size;
    } else {
      uint64_t decrypted_size = 0;
      if (rgw_get_gcm_decrypted_size(dpp, s->attrset, s->size, &decrypted_size)) {
...
int RGWRados::Object::Read::prepare(optional_yield y, const DoutPrefixProvider *dpp)
{
  ...
  if (params.part_num) {
    ...
    for (auto& iter : src_attrset) {
      /*
       * Skip object-level size attrs that represent the full object, not this
       * part. ORIGINAL_SIZE causes Content-Length mismatch; PARTS causes
       * rgw_get_gcm_decrypted_size() to sum all parts instead of this one.
       */
