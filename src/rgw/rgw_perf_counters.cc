...
void add_lc_counters(PerfCountersBuilder *pcb) {
  ...
  pcb->add_u64(l_rgw_lc_per_bucket_obj_processed, "objects_processed",
               "Objects processed (scanned) for LC in current run");
  ...
  pcb->add_u64(l_rgw_lc_per_bucket_obj_total, "objects_total",
               "Total objects processed in last completed LC run");
}
