...
public:
      ...
      const std::string& domain = "SSE-C-GCM")
  {
    ...
    bool hmac_ok = true;
    auto hmac_update_with_length = [&ctx, &hmac_ok](const std::string_view& data) {
      if (!hmac_ok) return;
      ...
      if (1 != HMAC_Update(ctx, len_buf, 4)) {
        hmac_ok = false;
        return;
      }
      if (!data.empty()) {
        if (1 != HMAC_Update(ctx, reinterpret_cast<const uint8_t*>(data.data()), data.size())) {
          hmac_ok = false;
        }
      }
    };
    ...
    if (1 != HMAC_Update(ctx, base_nonce, AES_256_IVSIZE)) {
      ldpp_dout(dpp, 0) << "ERROR: derive_object_key: HMAC_Update (nonce) failed" << dendl;
      HMAC_CTX_free(ctx);
      return false;
    }
    ...
    if (!hmac_ok) {
      ldpp_dout(dpp, 0) << "ERROR: derive_object_key: HMAC_Update failed" << dendl;
      HMAC_CTX_free(ctx);
      return false;
    }
...
RGWGetObj_BlockDecrypt::RGWGetObj_BlockDecrypt(const DoutPrefixProvider *dpp,
    ...
    starting_part_num(starting_part_num),
    current_part_num(starting_part_num)
{
  block_size = this->crypt->get_block_size();
  encrypted_block_size = this->crypt->get_encrypted_block_size();
  if (starting_part_num > 0) {
    this->crypt->set_part_number(starting_part_num);
  }
}
...
uint32_t RGWGetObj_BlockDecrypt::read_manifest_starting_part(
    const DoutPrefixProvider *dpp,
    const bufferlist& manifest_bl)
{
  ...
  auto mi = manifest.obj_begin(dpp);
  if (mi == manifest.obj_end(dpp)) {
    return 0;
  }
  return mi.get_cur_part_id();  // 0 for regular PUT, ≥1 for multipart
}
...
int RGWGetObj_BlockDecrypt::process_part_boundaries(size_t& plain_part_ofs_out) {
  ...
  const bool is_multipart = (starting_part_num > 0);
  uint32_t part_idx = 0;
  int res = 0;

  for (size_t part : parts_len) {
    uint32_t this_part_num = is_multipart ? (starting_part_num + part_idx) : 0;
    ...
    if (enc_part_ofs >= part) {
      ...
      // Move to next part
      part_idx++;
      uint32_t next_part_num = is_multipart ? (starting_part_num + part_idx) : 0;
                                ...
                                uint64_t* decrypted_size)
{
  ...
  if (auto i = attrs.find(RGW_ATTR_CRYPT_PARTS); i != attrs.end()) {
    ...
    if (!parts_len.empty()) {
      ...
      for (size_t enc_part : parts_len) {
        total += gcm_encrypted_to_plaintext_size(enc_part, dpp);
      }
  ...
  *decrypted_size = gcm_encrypted_to_plaintext_size(encrypted_size, dpp);
  if (dpp) {
    ldpp_dout(dpp, 20) << "GCM: calculated decrypted size " << *decrypted_size
