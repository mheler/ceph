...
* ``target_path`` (string)
  ...
  Supports the template variables ``${zonegroup}``, ``${storage_class}``,
  ``${tenant}``, and ``${owner}``. The ``${owner}`` variable expands to the
  bucket owner's identifier (user ID or account ID).

  The ``target_path`` value is only used when ``target_by_bucket`` is false.

  For example: ``target_path = rgwx-${owner}-archive/``

* ``target_by_bucket`` (boolean)

  If set to ``true``, each source bucket transitions to its own destination
  bucket instead of sharing a single target bucket. Defaults to ``false`` to
