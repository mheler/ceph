...
int restore_obj_from_cloud(RGWLCCloudTierCtx& tier_ctx,
  ...
  bool build_deferred_chain(const DoutPrefixProvider *dpp,
                            const RGWBucketInfo& bucket_info,
                            const rgw_obj& obj,
                            ...
                            std::string* tag);
  struct DeferredQueueResult {
    int ret{0};
    bool deleted_inline{false};
  };
  DeferredQueueResult queue_deferred_chain(const DoutPrefixProvider *dpp,
                                           cls_rgw_obj_chain& chain,
                                           const std::string& tag,
                                           const std::string& head_id_tag,
                                           optional_yield y);
  struct LCDeferredEnqueueResult {
    int ret{0};
    bool queued{false};
    bool deleted_inline{false};
  };
  LCDeferredEnqueueResult enqueue_lc_deferred_chain(
      const DoutPrefixProvider *dpp,
      const RGWBucketInfo& bucket_info,
      const rgw_obj& obj,
      RGWObjState* state,
  ...
  int recover_deferred_delete(const DoutPrefixProvider *dpp,
                              const RGWBucketInfo& bucket_info,
                              const rgw_obj& obj,
                              optional_yield y);
